...
```URDF``` and ```launch``` are edited to enable multi-robot simulation in Gazebo. May not work with physical robots.
...
```
$ cd ~/catkin_ws/src
$ git clone -b noetic-multirobot-devel https://github.com/Sungwwoo/jackal_kinova_simulator.git
...
$ roslaunch jackal_kinova_gazebo multi_test_world.launch

- Bring up MoveIt & RViz
$ roslaunch jackal_kinova_moveit_config multi_control.launch

- If you want to navigation using map, type the following command.
$ roslaunch jackal_kinova_navigation m_test_world.launch
```
